...
const float GatherResourceEvaluator::calculate_desirability(MovingEntity *t) {
    ...
    vec2 resource_position = rm->get_closest_resource(t->get_position(), rm->get_resource_type(t->get_job_type()));

    //if there is a resource close and we don't have maximum carrying capacity continue gathering;
    if(resource_position.distance(t->get_position()) < 1 && t->get_carrying() < 100) {
        return 1.0;
    }
    return 0.0;
}
