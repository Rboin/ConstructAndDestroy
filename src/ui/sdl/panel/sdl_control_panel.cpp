...
void SDLControlPanel::render(SDLRenderer *renderer, mat2 &transformations, float d) {
    ...
    if (firstRender || last_selected_building != p->selected_building) {
        ...
        if (p->selected_building != nullptr && p->selected_building->get_building_type() != RESEARCH) {
            this->add_component(new SDLEntityPanel(get_similar_representation(), p->selected_building));
        } else if (p->selected_building != nullptr && p->selected_building->get_building_type() == RESEARCH) {
            this->add_component(new SDLResearchPanel(get_similar_representation(),p->selected_building));
        } else {
